...
let state = {
    ...
    // 2D Grid Position & Rotation tracking
    playerPos: new THREE.Vector3(0, 0, 0),
    currentAngle: 0, 
    targetAngle: 0,
    
    // Path Generation Tracker
    pathGen: {
        x: 0, z: 0, y: 0, 
        angle: 0, 
        lastY: 0, 
        straightsSinceTurn: 0,
        nextTurnDir: 0,
        safeChunksRemaining: 3 // Guarantees a safe zone at the start of the game and after turns
    },
...
function createCementTexture() {
    ...
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = '#9e9e9e'; ctx.fillRect(0, 0, 256, 256);
    for (let i = 0; i < 10000; i++) {
        const x = Math.random() * 256, y = Math.random() * 256, size = Math.random() * 2 + 1;
...
const mats = {
    floor: new THREE.MeshStandardMaterial({ map: createTileTexture(), roughness: 0.8 }),
    wood: new THREE.MeshStandardMaterial({ color: 0x5C4033, roughness: 0.9 }),
    wall: new THREE.MeshStandardMaterial({ map: cementTex, roughness: 1.0 }),
    ceiling: new THREE.MeshStandardMaterial({ map: cementTex, roughness: 1.0 }),
    paper: new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 1.0 }),
    coin: new THREE.MeshStandardMaterial({ color: CONFIG.colors.coin, roughness: 0.5 }),
    particle: new THREE.MeshBasicMaterial({ color: 0x4CAF50 }) 
};
...
function spawnCrashParticles(position) {
    for(let i=0; i<35; i++) {
        ...
        const p = new THREE.Mesh(geos.particle, mat); 
...
function createArrowSign(direction) {
    ...
    const board = new THREE.Mesh(new THREE.BoxGeometry(4, 2.5, 0.2), new THREE.MeshStandardMaterial({color: 0x111111})); board.position.y = 5.5; group.add(board);
    const canvas = document.createElement('canvas'); canvas.width = 512; canvas.height = 256;
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = '#ffcc00'; ctx.font = 'bold 90px Arial'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(direction === 1 ? '⬅️ TURN' : 'TURN ➡️', 256, 128);
    const signTex = new THREE.CanvasTexture(canvas);
    const signPlane = new THREE.Mesh(new THREE.PlaneGeometry(3.8, 2.3), new THREE.MeshBasicMaterial({map: signTex, transparent: true}));
...
function createPaperStacks() {
    const group = new THREE.Group(), mat = mats.paper;
    for(let i=0; i<4; i++) {
        const height = 0.5 + Math.random() * 0.6;
        const stack = new THREE.Mesh(new THREE.BoxGeometry(0.6, height, 0.8), mat);
...
function createSecurityBarrier() {
    ...
    const armMat = new THREE.MeshStandardMaterial({ color: 0xff0000, roughness: 0.5 });
...
// Dodge Obstacle (Big Cabinet)
function createCabinet() {
    const group = new THREE.Group();
    const body = new THREE.Mesh(new THREE.BoxGeometry(2.6, 3.5, 1.4), mats.wood);
    body.position.y = 1.75; 
    group.add(body);
    const darkWood = new THREE.MeshStandardMaterial({ color: 0x3d2b1f, roughness: 0.9 });
    const split = new THREE.Mesh(new THREE.BoxGeometry(0.05, 3.3, 1.45), darkWood);
    split.position.y = 1.75;
    group.add(split);
    const silver = new THREE.MeshStandardMaterial({ color: 0xcccccc, roughness: 0.4, metalness: 0.8 });
    const handle1 = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.6, 0.15), silver);
    handle1.position.set(-0.2, 1.5, 0.7);
    const handle2 = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.6, 0.15), silver);
    handle2.position.set(0.2, 1.5, 0.7);
...
function createNBITexture() {
    ...
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = '#D4AF37'; ctx.font = 'bold 90px Arial, sans-serif';
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('NBI', 128, 64);
    return new THREE.CanvasTexture(canvas);
}

const legGeo = new THREE.CylinderGeometry(0.12, 0.1, 0.6); legGeo.translate(0, -0.3, 0); 
const armGeo = new THREE.CylinderGeometry(0.1, 0.08, 0.7); armGeo.translate(0, -0.35, 0); 

function createBato() {
    const group = new THREE.Group();
    group.rotation.order = 'YXZ'; 

    const skin = new THREE.MeshStandardMaterial({ color: 0xe0ac69, roughness: 0.6 });
    const barong = new THREE.MeshStandardMaterial({ color: 0xffffee, roughness: 0.9 });
    const pants = new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 0.8 });

    const head = new THREE.Mesh(new THREE.SphereGeometry(0.3, 16, 16), skin); head.position.set(0, 1.6, 0);
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.8, 0.4), barong); body.position.set(0, 1.0, 0);
    const lLeg = new THREE.Mesh(legGeo, pants); lLeg.position.set(-0.2, 0.6, 0);
    const rLeg = new THREE.Mesh(legGeo, pants); rLeg.position.set(0.2, 0.6, 0);
    const lArm = new THREE.Mesh(armGeo, barong); lArm.position.set(-0.45, 1.3, 0);
    const rArm = new THREE.Mesh(armGeo, barong); rArm.position.set(0.45, 1.3, 0);
...
function createAgent() {
    ...
    const skin = new THREE.MeshStandardMaterial({ color: 0xd29953, roughness: 0.6 });
    const suit = new THREE.MeshStandardMaterial({ color: 0x001133, roughness: 0.8 }); 
    const pants = new THREE.MeshStandardMaterial({ color: 0x00081a, roughness: 0.8 });
    ...
    const nbiMat = new THREE.MeshBasicMaterial({ map: createNBITexture(), transparent: true });
    const nbiPlane = new THREE.Mesh(new THREE.PlaneGeometry(0.6, 0.3), nbiMat); nbiPlane.position.set(0, 1.1, 0.226);
    const lLeg = new THREE.Mesh(legGeo, pants); lLeg.position.set(-0.2, 0.6, 0);
    const rLeg = new THREE.Mesh(legGeo, pants); rLeg.position.set(0.2, 0.6, 0);
    const lArm = new THREE.Mesh(armGeo, suit); lArm.position.set(-0.48, 1.3, 0);
    const rArm = new THREE.Mesh(armGeo, suit); rArm.position.set(0.48, 1.3, 0);
...
document.addEventListener('touchend', e => {
    ...
    if (Math.abs(diffX) > Math.abs(diffY)) { if (Math.abs(diffX) > 30) diffX > 0 ? handleLeftRightInput(-1) : handleLeftRightInput(1); } 
    else { if (Math.abs(diffY) > 30) diffY < 0 ? jump() : slide(); }
});
...
function createChunk(type = 'straight') {
    ...
    if (type === 'straight') {
        ...
        endY = startY;
        // When an intersection is spawned, guarantee the next 2 straight chunks are safe
    ...
    if (type === 'straight') {
        // Check if this straight chunk should be a safe zone
        let isSafe = false;
        if (state.pathGen.safeChunksRemaining > 0) {
            isSafe = true;
            state.pathGen.safeChunksRemaining--;
        }
...
function spawnEntities(chunkGroup, startY, endY, isSafe) {
    ...
    // Obstacles ONLY spawn if we are not on stairs, AND it is NOT a safe zone
    if (!isStairs && !isSafe) {
        if (Math.random() > 0.15) { 
            ...
            for(let i=0; i<numObs; i++) {
                ...
                if (rType > 0.7) { 
                    obsMesh = Math.random() > 0.5 ? createChair() : createPaperStacks(); 
                    obsType = 'jump'; 
                } 
                else if (rType > 0.4) { 
                    const r = Math.random(); 
                    if (r > 0.66) obsMesh = createTable(); 
                    else if (r > 0.33) obsMesh = createDesk(); 
                    else obsMesh = createSecurityBarrier(); 
                    obsType = 'slide'; 
                } 
                else {
                    obsMesh = createCabinet();
                    obsType = 'solid'; 
                }
    ...
    // Coins can always spawn safely
    const coinLane = lanes[isStairs ? 0 : 1];
    for(let i=-2; i<=2; i+=2) {
        if (Math.random() > 0.2) {
            const cProg = (CONFIG.chunkLength / 2 - i * 4) / CONFIG.chunkLength;
            const cY = startY + cProg * (endY - startY);
            const coin = new THREE.Mesh(geos.coin, mats.coin);
            coin.position.set(coinLane * -CONFIG.laneWidth, cY + 0.5, i * 4); chunkGroup.add(coin);
...
function resetGame() {
    ...
    // Reset pathGen completely. Start with 3 safe chunks!
    ...
    chaserGroup1.position.set(0, 0, 3); 
    chaserGroup2.position.set(0, 0, 6); 
...
function animate() {
    ...
    if (state.isPlaying) {
        ...
        for (let chunk of state.chunks) {
            ...
            if (dist <= CONFIG.chunkLength * 0.8) { 
                ...
                activeY = chunk.userData.startY + Math.max(0, Math.min(1, progress)) * (chunk.userData.endY - chunk.userData.startY);
                break;
            }
        }
        ...
        if (targetState1) {
            chaserGroup1.position.set(targetState1.x, targetState1.y, targetState1.z);
            chaserGroup1.rotation.set(targetState1.rotX, targetState1.angle, 0);
        }

        let targetState2 = state.history.length >= 30 ? state.history[state.history.length - 30] : null;
        if (targetState2) {
            chaserGroup2.position.set(targetState2.x, targetState2.y, targetState2.z);
            chaserGroup2.rotation.set(targetState2.rotX, targetState2.angle, 0);
        }

        const animateCharacter = (limbs, isJumping, isSliding) => {
            let tLL, tRL, tLA, tRA;
            if (isJumping) { 
                tLL = -0.8; tRL = 0.2; tLA = Math.PI * 0.8; tRA = Math.PI * 0.8; 
            } else if (isSliding) { 
                tLL = -0.1; tRL = -0.1; tLA = 0; tRA = 0; 
            } else { 
                const sprintSpeed = time * 1.2; 
                tLL = Math.sin(sprintSpeed) * 0.9; 
                tRL = Math.sin(sprintSpeed + Math.PI) * 0.9; 
                tLA = Math.sin(sprintSpeed + Math.PI) * 0.6; 
                tRA = Math.sin(sprintSpeed) * 0.6; 
            }
            limbs.lLeg.rotation.x += (tLL - limbs.lLeg.rotation.x) * 0.25; 
            limbs.rLeg.rotation.x += (tRL - limbs.rLeg.rotation.x) * 0.25;
            limbs.lArm.rotation.x += (tLA - limbs.lArm.rotation.x) * 0.25; 
            limbs.rArm.rotation.x += (tRA - limbs.rArm.rotation.x) * 0.25;
        };
    ...
    for(let i=state.particles.length-1; i>=0; i--) {
        ...
        p.userData.velocity.y += CONFIG.gravity * 0.5; 
